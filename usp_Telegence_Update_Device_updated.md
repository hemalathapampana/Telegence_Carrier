...
BEGIN
    ...
    -- Track all SubscriberNumbers seen this run
    SELECT DISTINCT SubscriberNumber INTO #SeenThisRun FROM #Source;
    ...
    -- Immediate Unknown for devices not present in a valid feed this run
    IF (@EnableImmediateUnknown = 1)
    BEGIN
        UPDATE td
        SET
            td.[OldDeviceStatusId]       = td.[DeviceStatusId],
            td.[DeviceStatusId]          = @UnknownStatusId,
            td.[SubscriberNumberStatus]  = 'Unknown',
            td.[EffectiveStatus]         = 'Unknown',
            td.[StatusReason]            = 'NOT_FOUND_IN_FEED',
            td.[ModifiedBy]              = 'usp_Telegence_Update_Device',
            td.[ModifiedDate]            = GETUTCDATE()
        FROM [dbo].[TelegenceDevice] td
        WHERE td.ServiceProviderId = @ServiceProviderId
          AND NOT EXISTS (SELECT 1 FROM #SeenThisRun s WHERE s.SubscriberNumber = td.SubscriberNumber);
    END

    -- Remove temporary artifacts
    DROP TABLE IF EXISTS #Source;
    DROP TABLE IF EXISTS #SeenThisRun;
